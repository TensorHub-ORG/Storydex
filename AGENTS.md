...
- 当前仓库协作只约束 `dev/windows` 与 `main`：Windows 改动先进入 `dev/windows` 并通过 Development CI，再进入 `main` 接受受影响组件检查。完整跨平台质量门禁仅用于显式 full CI、正式发布 workflow 或人工明确触发的验证。其他远端分支不在本文件治理范围内，不因日常任务主动清理、同步或改写。
- 普通 push 和 pull request 使用路径范围检查；完整质量门禁只在显式 full CI、正式发布 workflow 或人工明确触发时执行。本地完整套件仅在人工明确需要时运行，不得作为每次 push 的前置条件。
- 普通 push 和 pull request 不运行打包或 GUI E2E；这些检查只保留给显式 full CI 和正式发布 workflow。
...
- `dev/windows` 不得把轻量 CI 的成功表述为 release-ready；需要发布或完整验收时必须显式运行 full CI。
