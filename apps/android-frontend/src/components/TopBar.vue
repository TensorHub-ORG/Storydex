...
const contextConfirm = ref(false)
...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  scripts_time: '剧本与时间', progression: '剧情推进', constraints: '生成约束', retrieval_planning: '检索规划', player_interaction: '玩家交互', capabilities: '内置能力与检索',
...
function toggleUsage() { usageOpen.value = !usageOpen.value; modelOpen.value = false; usageNotice.value = ''; periodConfirm.value = false; contextConfirm.value = false }
function clearContext() {
  if (!session.clearContextWindow()) {
    usageNotice.value = '执行中无法清空，请先停止本轮'
    contextConfirm.value = false
    return
  }
  contextConfirm.value = false
  usageNotice.value = '当前模式的上下文窗口已清空'
}
  ...
  <header class="topbar">
    ...
    <div v-if="usageOpen" class="usage-menu">
      ...
      <template v-if="usageView === 'current'">
        ...
        <button class="context-clear" :disabled="session.isBusy" @click="contextConfirm = true"><CoomiIcon name="trash" :size="15" /><span>清空当前上下文</span></button>
        <p v-if="usageNotice" class="usage-notice">{{ usageNotice }}</p>
      </template>
      <template v-else>
      ...
      <div v-if="contextConfirm" class="period-confirm"><b>清空当前上下文？</b><p>仅清空当前项目、当前模式的模型上下文与界面记录，其他模式不受影响。</p><div><button @click="contextConfirm = false">取消</button><button class="danger" @click="clearContext">清空</button></div></div>
    </div>
...
.empty { margin:8px 0; color:var(--text-3); font-size:11.5px; text-align:center; }.period-button { width:100%; min-height:36px; border-radius:6px; background:var(--fill-strong); color:var(--text-2); font-size:12px; }.usage-notice { margin:7px 0 0; color:var(--ok); font-size:11px; }.period-confirm { margin-top:9px; padding:10px; border-radius:6px; background:var(--fill); }.period-confirm > b { font-size:12.5px; }.period-confirm p { margin:3px 0 9px; color:var(--text-3); font-size:11px; }.period-confirm > div { display:flex; gap:7px; }.period-confirm button { min-height:34px; flex:1; border-radius:5px; background:var(--bg); }.period-confirm button.primary { background:var(--blue); color:var(--on-accent); }
.context-clear { display:flex; align-items:center; justify-content:center; gap:6px; width:100%; min-height:36px; border-radius:6px; background:var(--fill-strong); color:var(--danger); font-size:12px; }.context-clear:disabled { opacity:.45; }.period-confirm button.danger { background:var(--danger); color:var(--on-accent); }
