...
<template>
  <header class="topbar">
    ...
    <button class="usage-button" :aria-expanded="usageOpen" aria-label="上下文与项目用量" @click="toggleUsage"><svg class="usage-ring" viewBox="0 0 36 36" aria-hidden="true"><circle class="usage-track" cx="18" cy="18" r="15" pathLength="100" /><circle class="usage-value" cx="18" cy="18" r="15" pathLength="100" :stroke-dasharray="usageStroke" /></svg><span>{{ usagePercent }}</span></button>
    ...
    <div v-if="usageOpen" class="usage-menu">
      ...
      <template v-else>
        ...
        <p v-if="!projectStats?.turns" class="empty">此模式在当前统计周期尚无数据</p>
        <button class="period-button" @click="periodConfirm = true">新建统计周期</button><p v-if="usageNotice" class="usage-notice">{{ usageNotice }}</p>
      </template>
      <div v-if="periodConfirm" class="period-confirm"><b>开始新统计周期？</b><p>当前周期不会删除，旧账本仍保留在项目中。</p><div><button @click="periodConfirm = false">取消</button><button class="primary" @click="newPeriod">新建</button></div></div>
    </div>
...
.usage-button { position:relative; }.usage-ring { width:31px; height:31px; transform:rotate(-90deg); }.usage-ring circle { fill:none; stroke-width:3.5; }.usage-track { stroke:var(--border-strong); }.usage-value { stroke:var(--blue); stroke-linecap:round; }.usage-button > span { position:absolute; font-size:8px; font-weight:700; color:var(--text-2); }
.usage-menu { right:8px; width:min(92vw,360px); max-height:min(76vh,620px); overflow-y:auto; padding:12px; }.usage-tabs,.mode-tabs { display:grid; grid-template-columns:repeat(2,1fr); padding:3px; border-radius:6px; background:var(--fill-strong); }.mode-tabs { grid-template-columns:repeat(3,1fr); margin:10px 0; }.usage-tabs button,.mode-tabs button { min-height:31px; border-radius:4px; color:var(--text-3); font-size:11.5px; }.usage-tabs button.on,.mode-tabs button.on { background:var(--bg); color:var(--blue); box-shadow:var(--shadow-1); }
