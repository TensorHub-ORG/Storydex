...
- 将 Coomi/OpenAI provider 冷启动初始化移出 SSE 事件循环，避免首次导入阻塞 heartbeat。
...
- 修复启动阶段旧历史响应覆盖正在运行会话、导致末尾 Git 提示丢失的竞态。
- Windows 安装包和便携包内置可迁移 Python 3.9、后端依赖与 MinGit，无需用户另行安装 Python/Git。
- 正式包自动排除测试目录、coverage、pytest 缓存、日志、`.env` 等开发期文件。
