...
$form.Add_Shown({
  try {
    ...
    $script:ExitCode = 1
    Write-UpdateLog "Installation failed: $($_.Exception.Message)"
    Set-InstallLock "failed"
    $progress.Style = "Blocks"; $progress.Value = 0
    $title.Text = "Storydex update failed"
    $detail.Text = "$($_.Exception.Message)`nThe existing version is preserved. Try again later or use the full installer."
    [Windows.Forms.Application]::DoEvents()
